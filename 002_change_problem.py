...
# 这道题的硬币可以使用无限次，自然而然的联想到完全背包问题，所以我就叫 完全硬币找零问题 了。
# 这篇题解，会使用背包问题的思想对硬币找零问题进行建模，按顺序对硬币找零系列问题进行说明。
#
# 一、 01硬币找零问题（01背包）
# 给定不同面额的硬币 coins 和总金额 m。每个硬币最多选择一次。计算可以凑成总金额所需的最少的硬币个数。
# 如果没有任何一种硬币组合能组成总金额，返回 -1。
#
# 状态表示：f[i][j] 表示只看前 i 个物品，总价值是 j 的情况下的最小硬币数目。
# 状态转移：f[i, j] = min(f[i-1, j], f[i-1, j-ci] + 1) 分别对应了不拿和拿第 i 个硬币两种情况。
#
...
# 二、完全硬币找零问题（完全背包）
# 给定不同面额的硬币 coins 和总金额 m。每个硬币可以选择无数次。计算可以凑成总金额所需的最少的硬币个数。
# 如果没有任何一种硬币组合能组成总金额，返回 -1。
#
# 状态表示：f[i][j] 为考虑前 i 种硬币，凑出金额为 j 的最少数目。
# 状态转移：考虑第 i 种硬币，我们可以不拿，或者拿 1...k 个，直到把金额拿爆。
